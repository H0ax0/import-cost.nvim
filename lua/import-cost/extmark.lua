...
local ic, job, util = require("import-cost"), require("import-cost.job"), require("import-cost.util")
...
function M.set_extmarks(bufnr)
	...
	local cmd = { "/home/hoax/.nvm/versions/node/v16.20.2/bin/node", ic.script_path, path, filetype }
...
function M.delete_extmarks(bufnr)
	...
	for string, data in pairs(cache[bufnr]) do
		if
			string
			== util.normalize_string(vim.api.nvim_buf_get_text(bufnr, data.line - 1, 0, data.line - 1, -1, {})[1])
		then
			goto continue
		else
